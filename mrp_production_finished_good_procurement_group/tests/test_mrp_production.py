...
class MrpProductionCase(TransactionCase):
    def setUp(self, *args, **kwargs):
        ...
        self.bom1.write({"routing_id": False})

    def _create_procurement_group(self, name):
        res = {"name": name, "move_type": "direct"}
    ...
    def test_no_create_group(self):
        ...
        for raw in mo.move_lines:
            self.assertEqual(raw.location_id, self.loc_stock_2)
            self.assertEqual(raw.location_dest_id, self.loc_production)
            self.assertEqual(raw.procure_method, "make_to_order")
            self.assertEqual(len(raw.group_id), 0)

    def test_create_group_manual(self):
        group1 = self._create_procurement_group("X 001")
        # Create MO
        mo = self._create_mo(group=group1.id)
        # Click confirm button
        mo.signal_workflow("button_confirm")
        for raw in mo.move_lines:
            self.assertEqual(raw.location_id, self.loc_stock_2)
            self.assertEqual(raw.location_dest_id, self.loc_production)
            self.assertEqual(raw.procure_method, "make_to_order")
            self.assertEqual(raw.group_id, group1)

    def test_create_group_auto(self):
        mo = self._create_mo(group=False, auto=True)
        # Click confirm button
        mo.signal_workflow("button_confirm")
        for raw in mo.move_lines:
            self.assertEqual(raw.location_id, self.loc_stock_2)
            self.assertEqual(raw.location_dest_id, self.loc_production)
            self.assertEqual(raw.procure_method, "make_to_order")
            self.assertEqual(raw.group_id, mo.raw_material_procurement_group_id)
    ...
    def test_copy_mo_1(self):
        ...
        self.assertNotEqual(
            mo.raw_material_procurement_group_id, mo2.raw_material_procurement_group_id
        )

        self.assertEqual(mo2.raw_material_procurement_group_id.id, False)
    ...
    def test_copy_mo_2(self):
        ...
        self.assertNotEqual(mo.raw_material_procurement_group_id.id, False)

        self.assertEqual(mo.raw_material_procurement_group_id.name, mo.name)
    ...
    def test_routing(self):
        ...
        self.bom1.write({"routing_id": routing.id})
        # Create MO
        mo = self._create_mo(auto=True)
